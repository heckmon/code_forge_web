...
///
/// [CodeForgeWeb] provides a feature-rich code editing experience with support for:
...
/// )
/// ```
class CodeForgeWeb extends StatefulWidget {
  ...
  ///
  /// When provided with an LSP-configured controller, automatically initializes
  /// LSP features including completions, semantic highlighting, and diagnostics.
  /// For web compatibility, use 'file://' URIs with virtual paths.
  final String? fileUri;

  /// A raw file URL to load content from (e.g., GitHub raw URL).
  ///
  /// When provided, the widget will automatically fetch content from this URL
  /// and display it in the editor. This is useful for loading remote files.
  /// Cannot be used together with [initialText].
  final String? fileUrl;

  /// Initial text content for the editor.
  ///
  /// Used only during initialization; subsequent changes should use
  /// the controller. Cannot be used together with [fileUrl].
  ...
  /// The type of the virtual keyboard that will used by the [CodeForgeWeb].
  ///
  ...
  /// Creates a [CodeForgeWeb] code editor widget.
  const CodeForgeWeb({
    super.key,
    this.controller,
    this.undoController,
  ...
  @override
  State<CodeForgeWeb> createState() => _CodeForgeWebState();
}

class _CodeForgeWebState extends State<CodeForgeWeb>
  ...
  late final VoidCallback _semanticTokensListener, _controllerListener;
  ...
  void initState() {
    super.initState();
    _controller = widget.controller ?? CodeForgeWebController();
    ...
    if (_filePath == null &&
      ...
      try {
        ...
        final filename = uri.pathSegments.isNotEmpty
            ...
            : 'untitled.txt';
        final workspacePath = _controller.lspConfig!.workspacePath;
        _filePath = '$workspacePath/$filename';
      } catch (e) {
        debugPrint('Error parsing fileUrl: $e');
      }
    }

    if (_filePath == null && _controller.lspConfig != null) {
      _filePath = 'file:///workspace/untitled.${_language.name ?? 'txt'}';
    }
    ...
    if (widget.fileUrl != null) {
      _loadContentFromUrl(widget.fileUrl!);
    } else if (widget.initialText != null && widget.initialText!.isNotEmpty) {
      _controller.text = widget.initialText!;

      if (_filePath != null &&
          _controller.lspConfig != null &&
          !_controller.lspConfig!.isInitialized) {
        _initializeLsp();
      }
  ...
  Future<void> _loadContentFromUrl(String url) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        debugPrint('Failed to load content from URL: ${response.statusCode}');
        if (mounted) {
          _controller.text =
              '// Failed to load file: HTTP ${response.statusCode}';
        }
      }
    } catch (e) {
      debugPrint('Error loading content from URL: $e');
      if (mounted) {
  ...
  Future<void> _initializeLsp() async {
    if (_filePath == null || _controller.lspConfig == null) return;

    try {
      final lspConfig = _controller.lspConfig!;

      if (lspConfig is LspSocketConfig && !lspConfig.isInitialized) {
      ...
      _controller.markLspReady();

      if (mounted) {
        final tokens = await lspConfig.getSemanticTokensFull(_filePath!);
        if (mounted && tokens.isNotEmpty) {
          _controller.semanticTokens.value = (tokens, 0);
        }

        await _controller.fetchDocumentColors();
      }
    } catch (e) {
...
class _CodeFieldRenderer extends RenderBox implements MouseTrackerAnnotation {
  ...
  bool _isCachedHeightExact = false, _caretSyncAfterLayoutScheduled = false;
  ...
  int _previousLineCount = 0, _virtualRemovedTotalLineCount = 0;
  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    for (int i = firstVisibleLine; i <= lastVisibleLine && i < lineCount; i++) {
      ...
      final textX = isRTL
          ? (innerPadding?.left ?? 0) - (lineWrap ? 0 : _effectiveHScroll)
          : _gutterWidth +
                (innerPadding?.left ?? 0) -
                (lineWrap ? 0 : _effectiveHScroll);
        ...
        if (lspActionNotifier.value != null && lspConfig != null) {
          ...
          if (actions.any((item) {
            try {
              ...
              try {
                final fileUri = filePath!.startsWith('file://')
                    ? filePath
