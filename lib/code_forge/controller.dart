...
/// Controller for the [CodeForgeWeb] code editor widget.
///
...
/// ```
class CodeForgeWebController implements DeltaTextInputClient {
  ...
  CodeForgeWebController({this.lspConfig}) {
    if (lspConfig != null) {
      (() async {
        try {
          ...
          if (filePath != null) {
            await lspConfig!.openDocument(filePath, initialContent: text);
            _lspReady = true;
      ...
      _listeners.add(() async {
        ...
        _debounceTimer = Timer(const Duration(milliseconds: 200), () async {
          if (text != _previousValue) {
            _wordCache = await compute(
              CodeForgeWebController._extractWords,
              text,
            );
          }
          _previousValue = text;
  ...
  ///
  /// Note: File system operations are not supported on web.
  /// Use [text] getter to get the content and handle saving in your application.
  void saveFile() {
    if (openedFile == null) {
      throw FlutterError(
  ...
  ///
  /// Note: File system operations are not supported on web.
  /// Set the [text] property directly to update content.
  void refetchFile() {
    throw UnsupportedError(
  ...
  Future<void> applyWorkspaceEdit(dynamic action) async {
    ...
    final fileUri = openedFile!.startsWith('file://')
        ? openedFile
